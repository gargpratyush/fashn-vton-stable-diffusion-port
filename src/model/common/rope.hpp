...
namespace Rope {
    ...
    __STATIC_INLINE__ ggml_tensor* attention(GGMLRunnerContext* ctx,
        ...
        auto x = ggml_ext_attention_ext(ctx->ggml_ctx, ctx->backend, q, k, v, n_head, mask, true, ctx->flash_attn_enabled, kv_scale, ctx->flash_attn_f32_kv, ctx->attention_fallback_device);  // [N, L, n_head*d_head]
        return x;
    }
};  // namespace Rope
